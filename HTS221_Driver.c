...
static HAL_StatusTypeDef HTS221_ReadReg(uint16_t RegAddr, uint16_t NumByteToRead, uint8_t *Data) {
  HAL_StatusTypeDef s;
  RegAddr = RegAddr | 0x80;
  s =  HAL_I2C_Mem_Read(&hi2c1, HTS221_I2C_ADDRESS , RegAddr, 1, Data, NumByteToRead, 1000);  
  return s;
}

static HAL_StatusTypeDef HTS221_WriteReg(uint16_t RegAddr, uint16_t NumByteToWrite, uint8_t *Data) {
  HAL_StatusTypeDef s;
  RegAddr = RegAddr | 0x80;
  s =  HAL_I2C_Mem_Write(&hi2c1, HTS221_I2C_ADDRESS , RegAddr, 1, Data, NumByteToWrite, 1000);
  return s;
}
...
HTS221_Error_et HTS221_Get_Humidity(uint16_t* value)
{
  ...
  tmp = ((int32_t)(H_T_out - H0_T0_out)) * ((uint32_t)(H1_rh - H0_rh)*10);
